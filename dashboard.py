...
class Plotly():
    ...
    @classmethod
    def graph_number(cls, date):
        fig = data.data_rea(date)

        return fig

    @classmethod
    def graph_bar(cls, date):
        fig = px.bar(data.data_bar(date), x=data.data_bar(date).index, y='incid_rea', 
            color=data.data_bar(date).index, title="Total de l'incidence en réanimation par région", 
            labels={"incid_rea":"Total de personnes en réanimation", 'color':'Légende', 'x':'Régions'})
...
#Plotly.graph_line()
# Métropole
#Plotly.graph_map(47, 3, 4.2, '2020-03-19')
# Guyane
